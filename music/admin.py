...
def admin_get_user_list():
	...
	return render_template('admin/get_user.html', content= tuple(userlist))

@app.route('/admin_all_artist_list')
def admin_get_artist_list():
	artistlist = enduser.find({'isapproved': True, 'role':{'$ne':None}})
	return render_template('admin/get_artist.html', content= tuple(artistlist))

@app.route('/admin_approve_artist')
def admin_approve_artist():
	approvelist = enduser.find({'isapproved': False, 'role':{'$ne':None}})
	return render_template('admin/approve_artist.html', content= tuple(approvelist))

@app.route('/admin_get_file_list')
def admin_get_file_list():
	filelist = uploads.find()
	return render_template('admin/files.html', content= tuple(filelist))

@app.route('/approve_artist/<id>')
def approve_artist(id):
	get = {'_id':int(id)}
	setv = {"$set": {"isapproved": True}}
	getuser = enduser.update_one(get, setv)


	return redirect(url_for('admin_approve_artist'))
